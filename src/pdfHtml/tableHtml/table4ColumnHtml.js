	...
	const template = `
		<div class="table four">
			<div class="row-header">
				...
				<div class="header dynamic"><%= dynamicCol2 %></div>
				<div class="header dynamic"><%= dynamicCol3 %></div>
				<div class="header dynamic"><%= dynamicCol4 %></div>
			</div>
			...
			<div class="row-data">
				<div class="item-wrapper">
					<div class="item">
						...
						<div class="item-data dynamic">
							<% itemReq.dynamicVal2.forEach((dynamicVal)=> { %>
							<%= dynamicVal.val %>
							<% }); %>
						</div>
						<div class="item-data dynamic">
							<% itemReq.dynamicVal3.forEach((dynamicVal)=> { %>
							<%= dynamicVal.val %>
							<% }); %>
						</div>
						<div class="item-data dynamic">
							<% itemReq.dynamicVal4.forEach((dynamicVal)=> { %>
							<%= dynamicVal.val %>
							<% }); %>
						</div>
					</div>
